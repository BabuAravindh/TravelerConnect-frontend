...
export default function BookingsPage() {
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/bookings/admin/delete/${id}`, {
        method: 'DELETE',
      });
                  ...
                  <tr key={booking.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button
                        ...
                        className={`mr-2 ${deletingId === booking.id ? 'bg-gray-400' : 'bg-red-600 hover:bg-red-700'} text-white py-1 px-3 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500`}
                      >
